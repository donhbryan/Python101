...
def save_song_attrs_to_id3tags(tags, song):
    ...
    if song.title:
        tags["TIT2"] = TIT2(
            encoding=3, text=song.title.decode('unicode-escape'))

    # - TPE1: Artist
    if song.artist:
        tags["TPE1"] = TPE1(
            encoding=3, text=song.artist.decode('unicode-escape'))

    # - TDRC: Year
    if song.year:
        tags["TDRC"] = TDRC(encoding=3, text=str(
            song.year).decode('unicode-escape'))

    # - TALB: Album
    if song.album:
        tags["TALB"] = TALB(
            encoding=3, text=song.album.decode('unicode-escape'))
    ...
    if song.album_artist:
        tags["TPE2"] = TPE2(
            encoding=3, text=song.album_artist.decode('unicode-escape'))

    # - TRCK: Track number
    if song.track_number:
        tags["TRCK"] = TRCK(encoding=3, text=str(
            song.track_number).decode('unicode-escape'))

    # - TBPM: BPM
    if song.bpm:
        tags["TBPM"] = TBPM(encoding=3, text=str(
            song.bpm).decode('unicode-escape'))

    # - TOPE: Original artist
    if song.original_artist:
        tags["TOPE"] = TOPE(
            encoding=3, text=song.original_artist.decode('unicode-escape'))
    ...
    if song.composer:
        tags["TCOM"] = TCOM(
            encoding=3, text=song.composer.decode('unicode-escape'))

    # - TEXT: Lyricist
    if song.lyricist:
        tags["TEXT"] = TEXT(
            encoding=3, text=song.lyricist.decode('unicode-escape'))

    # - COMM: Comments
    if song.comments:
        tags["COMM"] = COMM(
            encoding=3, text=song.comments.decode('unicode-escape'))

    # - TPE4: Remixer
    if song.remixer:
        tags["TPE4"] = TPE4(
            encoding=3, text=song.remixer.decode('unicode-escape'))

    # - TPUB: Label/Publisher
    if song.label:
        tags["TPUB"] = TPUB(
            encoding=3, text=song.label.decode('unicode-escape'))

    # - TCON: Genre/content type
    if song.genre:
        tags["TCON"] = TCON(
            encoding=3, text=song.genre.decode('unicode-escape'))

    if song.lyrics:
        tags["USLT"] = USLT(
            encoding=3, text=song.lyrics.decode('unicode-escape'))
